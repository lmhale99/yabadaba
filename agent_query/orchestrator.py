...
def wants_multi_console() -> bool:
    return os.getenv("ORCH_MULTI_CONSOLE", "1").lower() not in {
        "0",
        "false",
        "no",
        "off",
...
def get_venv_dir() -> Path | None:
    """
    Resolve the virtual environment directory.

    Priority:
    1. ORCH_VENV_DIR
    2. Nearby .venv directories
    3. Active virtual environment, if one is active
    """
...
def find_executable(name: str, venv_bin: Path) -> str:
    """
    Find an executable in the virtual environment first, then PATH.
    Automatically handles .exe on Windows.
    """
    ...
    raise FileNotFoundError(
        ...
        f"Checked {venv_executable} and system PATH."
    )
...
def get_open_webui_data_dir() -> Path:
    configured = (
        os.getenv("ORCH_OPEN_WEBUI_DATA_DIR")
        or os.getenv("OPEN_WEBUI_DATA_DIR")
    )
...
def create_posix_launcher_script(
    cmd: list[str],
    env: dict,
    title: str,
) -> tuple[Path, Path]:
    """
    Create a temporary shell script for macOS/Linux terminal windows.

    The script:
    - exports the desired environment
    - writes its PID to a pidfile
    - execs the real command so the PID remains the service PID

    Note:
    The temp launcher contains environment variables, so it is created with
...
def launch_in_new_windows_console(
    cmd: list[str],
    env: dict,
    title: str,
) -> ManagedProcess:
...
def launch_in_macos_terminal(
    ...
    osa_cmd = [
        "osascript",
        "-e",
        f'tell application "Terminal" to do script {applescript_quote(terminal_command)}',
        "-e",
...
def find_linux_terminal_command(
...
def launch_in_linux_terminal(
    ...
    if terminal_cmd is None:
        print(
            f"No supported Linux terminal emulator found for {title}. "
            "Falling back to attached process."
        )
...
def launch_attached(
    ...
    """
    Portable fallback that does not open a new terminal.
    """
...
def wait_for_managed_process(process: ManagedProcess):
    """
    Wait for either:
    - the subprocess handle on Windows/attached mode, or
    - the PID written by the POSIX launcher script.
    """
    if process.pidfile:
...
def stop_process(process: ManagedProcess):
    ...
    Windows:
      Stops the Popen process.

    macOS/Linux with a terminal launcher:
      Reads the service PID from the pidfile and terminates that process.

    Attached POSIX fallback:
...
def build_mcp_command(db_name: str | None = None) -> tuple[list[str], dict]:
    ...
    mcp_server_script = SCRIPT_DIR / os.getenv(
        "ORCH_MCP_SERVER_SCRIPT",
        "mcp_server.py",
    )

    database_name = db_name or os.getenv("ORCH_DATABASE_NAME", "default_db")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Cross-platform orchestrator for MCP and Open WebUI."
    )
...
def main():
    ...
    if args.mode == "mcp":
        ...
        process = start_mcp(db_name=db_name)

        try:
            wait_for_managed_process(process)
    ...
    try:
        processes.append(start_mcp())
        processes.append(start_webui())
